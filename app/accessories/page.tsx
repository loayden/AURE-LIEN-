...
function Orbs() {
  return (
    <div className="pointer-events-none fixed inset-0 overflow-hidden z-0">
      ...
      <div style={{
        position: "absolute", width: 500, height: 500, bottom: "15%", left: "-8%",
        background: "radial-gradient(circle, rgba(160,140,220,0.05) 0%, transparent 65%)",
        filter: "blur(80px)",
        animation: "orbB 30s ease-in-out infinite",
      }} />
      <style>{`
        @keyframes orbA { 0%,100%{transform:translate(0,0)} 50%{transform:translate(-40px,30px)} }
        @keyframes orbB { 0%,100%{transform:translate(0,0)} 50%{transform:translate(50px,-25px)} }
      `}</style>
    </div>
  );
}
  ...
  return (
    <>
      ...
      >
        ...
        {/* ── COLLECTION GRID ── */}
        <section ref={parallaxRef} className="relative z-10 py-10 pb-28 px-6 sm:px-12 max-w-6xl mx-auto">
